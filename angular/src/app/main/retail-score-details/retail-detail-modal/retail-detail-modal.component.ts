...
@Component({
  selector: 'retailDetlModal',
  templateUrl: './retail-detail-modal.component.html',
  styles: [
  ]
})
export class RetailDetailModalComponent extends AppComponentBase implements OnInit {
...
getSelitemValueAssets(){
  ...
  save(){
    ...
    this.retail.score = this.genderValue + this.ageValue + this.savingValue +
     ...
     this._service.editRetailScore(this.retail).subscribe(()=>{
